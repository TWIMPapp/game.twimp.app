...
export class EasterEventAPI {
    ...
    static async setCustomTrail(userId: string, locations: Array<{ lat: number; lng: number }>) {
        return this.nextAPI.post({ game_ref: 'easter-event', user_id: userId, action: 'set-custom-trail', locations }, {});
    }

    static async clearCustomTrail(userId: string) {
        return this.nextAPI.post({ game_ref: 'easter-event', user_id: userId, action: 'clear-custom-trail' }, {});
    }

    static async sendHelp(userId: string, message: string) {
        return this.nextAPI.post({ game_ref: 'easter-event', user_id: userId, action: 'help', message }, {});
    }
