...
import { AdventureAntsTask } from '@/typings/Task';
import { useEffect, useState } from 'react';
import { Position } from '@/hooks/useGeolocation';
import { useGeolocation } from '@/hooks/useGeolocation';
import AdventureAntsGame from '@/components/AdventureAntsGame';
...
export default function AdventureAnts({ testTask }: { testTask?: AdventureAntsTask }) {
  const [task, setTask] = useState<AdventureAntsTask>();
  ...
  useEffect(() => {
    const fetchData = () => {
      ...
      if (_params) {
        setParams(_params);
        const data = new TaskHandlerService().getTaskFromSession<AdventureAntsTask>();
          ...
          <div className="flex-1 flex items-center justify-center p-4">
            <div className="bg-white p-8 rounded-lg shadow-md text-center">
              <h1 className="text-3xl font-bold text-gray-800 mb-6">
                {score > -1 ? (score === 1 ? 'You Made It Home!' : 'Try Again!') : 'Adventure Ants'}
              </h1>
              <p className="text-gray-600 mb-6">
                Guide your ant back to the colony while avoiding obstacles!
              </p>
              <p className="text-gray-600 mb-6">You have 2 minutes to reach the colony</p>
            ...
            <div className="flex-1 relative h-[calc(100vh-4rem)]">
              {position && task && (
                <AdventureAntsGame
                  ...
                  colonyPosition={task.colonyPosition}
